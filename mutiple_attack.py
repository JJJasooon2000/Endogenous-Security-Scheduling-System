...
def update(results):  # 更新参数
    ...
    if len(results[1]) == 0:  # 未被攻击情况
        set_time += 0.01  # 对调度时间阈值进行调整
        for i in range(len(results[0])):
            all_run_time[results[0][i]] += 1  # 对在线执行体进行在线时间更新
            if reliability[results[0][i]] < 1:  # 对执行体安全度进行调整
                reliability[results[0][i]] += 0.1
    else:  # 受到攻击情况
        set_time = set_time / 2  # 调度时间阈值降为原来的一半
        for j in range(len(results[0])):  # 多模判决优势方，可信度降
            all_run_time[results[0][j]] += 1
            reliability[results[0][j]] = reliability[results[0][j]] / 1.2
        for k in range(len(results[1])):  # 多模判决劣势方，可信度降
            all_run_time[results[1][k]] += 1
            reliability[results[1][k]] = reliability[results[1][k]] / 2
    return 0
...
def scheduler(old_plan, results):  # 调度执行函数，决定下一轮组合
    ...
    if len(old_plan) == 0:  # 系统开机，初始方案随机决定
        ...
        server_num = 3
        find_threshold = np.array(reliability)
        find_threshold.sort()  # 找到执行体池中可信度的中位数，作为可信度阈值
        random_seed = random.randint(0, 4)  # 随机种子
        while reliability[random_seed] < find_threshold[2]:  # 可信度低于阈值的种子舍弃
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        while whole_time <= 999:
            ...
            test_web = ['baidu.com', 'bilibili.com', '']
            plan = scheduler(plan, result)
            # print("当前调度方案为:", plan)
            if whole_time == 100:
                domain_name_m = test_web[1]
            elif whole_time == 900:
                domain_name_m = test_web[2]
            else:
                domain_name_m = test_web[0]
        ...
        if ss == 3:
            second_pass += 1
    print(none_pass, first_pass, second_pass, two_pass)